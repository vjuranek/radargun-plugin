...
* `gatherLogs`: `True`/`False` determines whether logs should (not) be gathered from given machine. Default is `True`. Logs are sent to Jenkins master during the tests, so in case of excessive logging the bandwidth of the line can be reduced, which can impat the tests.
